...
The most important steps of our Mars Rover project were  in the beginning the decision to create a DSL in form of a state machine. With this decision we solved some problems we encountered during the project of the small Rover. For example the problem of the unclear working with the Arbitrator and the bluetooth communication using a Thread. The coding was for us now more open and we knew in every state exactly what the robot does, that made the programming and working with the robot more easy. But the decision created also some problems. We had to begin from scratch and couldn't use much code from the small Rover, we had to built the framework which was a challenge for us, because we had not much experience with LeJos and NXT. 
%Something about DSL of the small rover???

\subsection{Planning}
We tried to work every week one day together, but we had of course some problems, because we had not on the same day time to work together, but we managed to create a good task allocation and we could also work on different days. We also discussed every week a goal which should been reached, such that we could show a result to Jozef. As an effect of this is that we didn't stay to long on one complex problem and to find sometimes a solution which is more convenient and for that moment useful, but not really nice. \\

To get the project more structured we made a list with subject which can be problematic and which need more time to find a good solution. This helped because we got early an impression how big th project becomes and we had in the beginning time to get in the subjects. A second advantage of this approach was  that we could build our DSL around the difficult subjects and we didn't had to change the DSL so much. \\
During the hole programming phase we build small tests which are successive such that we could very nicely test every component separately. Further could we built the test that go more and more to a complete solution for the Mars Rover.\\
