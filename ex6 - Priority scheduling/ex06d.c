...
void demo(void *arg)
{
    ...
    runtime = 0;
    int prio_changed = 0;
    rt_task_inquire(rt_task_self(),&curtaskinfo);
    int priority = curtaskinfo.cprio;
    while(runtime < EXECTIME) {
    //if this is the high-priority task, check if it's halfway
    if(prio_changed < 1 && runtime > EXECTIME/2){
    	//if it's halfway, change the priorities of the other tasks.
	rt_task_set_priority(&demo_task[num],priority-12);
	rt_printf("Changed priority of task %d\n",num);
	prio_changed = 1;
	}
